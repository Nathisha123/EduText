...
const Navbar = () => {
    ...
    return (
        <nav className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all bg-transparent `}>
            ...
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                {/* logo */}
                <Link to="/">
                    <p className='text-3xl font-bold'>🎒 EduText</p>
                </Link>
                            ...
                            <li key={index}>
                                {
                                    link.title === "Catalog" ? (
                                        <div
                                            className={`group relative flex cursor-pointer items-center gap-1 py-2 px-5 rounded-xl ${matchRoute("/catalog/:catalogName")
                                        ...
                                        <Link to={link?.path}>
                                            <p className={`${matchRoute(link?.path) ? "bg-purple-800 text-white" : "text-richblack-25"} rounded-xl py-2 px-5 `}>
                                                {link.title}
                                            </p>
                                        </Link>)
                                }
                        ...
                        token === null && (
                            <Link to="/login">
                                ...
                                <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/login') ? 'border-[2.5px] border-purple-800' : 'border border-richblack-700 bg-richblack-800'} `}
                                >
                                    Log in
                                </button>
                            </Link>
                    ...
                    }
                    {
                        token === null && (
                            <Link to="/signup">
                                ...
                                <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/signup') ? 'border-[2.5px] border-purple-800' : 'border border-richblack-700 bg-richblack-800'} `}
                                >
                                    Sign Up
                                </button>
                            </Link>
